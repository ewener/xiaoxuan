...
public class DailySodetailInterceptor extends AbsInterceptorDefaultAdapter{
	...
	public void beforeDisplayReportData(ReportRequest rrequest, ReportBean rbean, ReportDataBean reportDataBean) {
		  ...
		  String begindate=rrequest.getStringAttribute("begindate","");
		   String enddate=rrequest.getStringAttribute("enddate","");
		    ...
		    String employeeno = rrequest.getStringAttribute("employeeno","");
		  ...
		  if(employee.equals("-9999")&&"".equals(employeeno.trim()))
		  {
		  		 wr.getMessageCollector().warn("受理员工和员工工号需填入一项！","{width:160,height:100}",false);
		  		 wr.terminateResponse(Consts.STATECODE_FAILED);
		  }
		  if(!employee.equals("-9999")&&!"".equals(employeeno.trim()))
		  {
		  		 wr.getMessageCollector().warn("受理员工和员工工号只能填入一项！","{width:160,height:100}",false);
		  		 wr.terminateResponse(Consts.STATECODE_FAILED);
		  }
